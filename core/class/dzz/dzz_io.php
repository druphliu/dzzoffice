...
class dzz_io
{
	...
	//获取缩略图
	function getThumb($path,$width,$height,$original){
		if($io=self::initIO($path)) $io->getThumb($path,$width,$height,$original);
	}
	/*
	...
	*/
	...
	function CreateFolder($path,$fname,$perm=0){
		if($io=self::initIO($path))	return $io->CreateFolder($path,$fname,$perm);
		else return false;
	}
	...
	function setFileContent($path,$data){
		if($io=self::initIO($path))	return $io->setFileContent($path,$data);
		else return false;
	}
