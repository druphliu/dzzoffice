...
if($perm<1) showmessage('没有权限',dreferer());
/*if(empty($_G['uid'])) {
...
if(submitcheck('edit')){
	...
	if($news['newid']){
		...
		showmessage('do_success',$forward,array('data'=>rawurlencode(json_encode(array('error'=>'发布失败','forward'=>$forward)))),array('showmsg'=>true));
	}
}else{
	$navtitle='';
