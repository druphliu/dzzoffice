		...
		if($bz=='all'){//获取所有的顶级目录，包括企业盘，网盘，云存储 和机构和部门；
			...
			//获取企业盘和组织机构
				...
				foreach($orglist as $value){
					...
					$folder=C::t('folder')->fetch($value['fid']);
			...
			if($folder=C::t('folder')->fetch_by_fid($id)){
				...
				if($folder['flag']=='recycle'){
					/*if($total=C::t('icos')->fetch_all_isdelete(0,'','',0,'',true)){
						...
						foreach(C::t('icos')->fetch_all_isdelete($perpage,$orderby,$order,$start,$uid) as $value){
							if($value['type']=='folder'){
								$folderids[]=$value['oid'];
					...
					/*if($total=C::t('icos')->fetch_all_by_pfid($folder['fid'],$folder,'',0,'','',0,true)){
							...
							foreach(C::t('icos')->fetch_all_by_pfid($folder['fid'],$keyword,$perpage,$orderby,$order,$start,$uid) as $value){
